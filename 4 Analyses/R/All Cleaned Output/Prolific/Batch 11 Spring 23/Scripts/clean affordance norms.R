...
##read in data
master = read.csv("0-Data/Merged_Prolific_Batch11.csv", stringsAsFactors = F)
...
dat$affordance_response[dat$affordance_response == "depends on what"] = NA
dat$affordance_response[dat$affordance_response == "what on earth do you mean"] = NA
dat$affordance_response[dat$affordance_response == "not many uses for it"] = NA
dat$affordance_response[dat$affordance_response == "not a clue... cooking?"] = NA
dat$affordance_response[dat$affordance_response == "not many uses for it"] = NA
dat$affordance_response[dat$affordance_response == "isnt that spanish?"] = NA
...
lemmatized[1303, 9] = "snot"

lemmatized = lemmatized[-1304, ]
...
##Write to .csv
#write.csv(combined, file = "Prolific Batch 11 Cleaned.csv", row.names = F)
