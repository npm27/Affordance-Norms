...
##read in data
master = read.csv("0-Data/Merged_Prolific_Batch3.csv", stringsAsFactors = F)
...
dat$affordance_response[dat$affordance_response == "cannot \"use\" newlyweds for anything "] = NA
...
dat$affordance_response[dat$affordance_response == "what kind of hardware?"] = NA
dat$affordance_response[dat$affordance_response == "what is this"] = NA
dat$affordance_response[dat$affordance_response == "a what?"] = NA
dat$affordance_response[dat$affordance_response == "again not an object"] = NA
dat$affordance_response[dat$affordance_response == "this is not an object?"] = NA
...
##Write to .csv
#write.csv(combined, file = "Prolific Batch 3 Cleaned.csv", row.names = F)
