...
##read in data
master = read.csv("0-Data/Merged_Prolific_Batch5.csv", stringsAsFactors = F)
...
dat$affordance_response[dat$affordance_response == "not sure what hibachi is."] = NA
dat$affordance_response[dat$affordance_response == "make money, talk to, learn from, this is not an object :) "] = NA
dat$affordance_response[dat$affordance_response == "this is not an object?"] = NA
dat$affordance_response[dat$affordance_response == "cannot be used"] = NA
dat$affordance_response[dat$affordance_response == "i do not know what this means - is it like a pillowcase?"] = NA
dat$affordance_response[dat$affordance_response == "not an object"] = NA
dat$affordance_response[dat$affordance_response == "i am not sure what a patrolman does - we don't have them in the uk"] = NA
dat$affordance_response[dat$affordance_response == "unknown"] = NA
dat$affordance_response[dat$affordance_response == "no uses come on~!!!!!"] = NA
dat$affordance_response[dat$affordance_response == "i don't know what that is,"] = NA
dat$affordance_response[dat$affordance_response == "it depends what type of caseworker"] = NA
dat$affordance_response[dat$affordance_response == "i don;t know what that is,"] = NA
dat$affordance_response[dat$affordance_response == "no idea what this is"] = NA
dat$affordance_response[dat$affordance_response == "wtf kind of a question is this, go insane, look all sense of reality, make a cake, get a job, get married, have a kid, die"] = NA
dat$affordance_response[dat$affordance_response == "cleaning,must leave soon this is really unenjoyable!"] = NA
...
##Write to .csv
#write.csv(combined, file = "Prolific Batch 5 Cleaned.csv", row.names = F)
