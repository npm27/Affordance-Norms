...
##read in data
master = read.csv("0-Data/Merged_Prolific_Batch8.csv", stringsAsFactors = F)
...
dat$affordance_response[dat$affordance_response == "not sure what u mean by fixture"] = NA
dat$affordance_response[dat$affordance_response == "is a nightstick what police use?"] = NA
dat$affordance_response[dat$affordance_response == "again no idea what this is"] = NA
dat$affordance_response[dat$affordance_response == "watching, potentially petting (dependent on what you mean by that word), guarding, protecting"] = NA
dat$affordance_response[dat$affordance_response == "this isn't an object"] = NA
dat$affordance_response[dat$affordance_response == "it's not an object, it's an action in itself"] = NA
dat$affordance_response[dat$affordance_response == "besides their job not muchc"] = NA
...
##Write to .csv
#write.csv(combined, file = "Prolific Batch 8 Cleaned.csv", row.names = F)
