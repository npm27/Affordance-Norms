...
##read in data
master = read.csv("0-Data/Merged_UCONN_12_29.csv", stringsAsFactors = F)
...
dat$affordance_response[dat$affordance_response == "unsure what this is"] = NA
dat$affordance_response[dat$affordance_response == "not sure what this is, maybe to hold items?"] = NA
dat$affordance_response[dat$affordance_response == "also not sure what this is. used in a car? "] = NA
dat$affordance_response[dat$affordance_response == "n/a"] = NA
...
##Write to .csv
#write.csv(combined, file = "UCONN  Cleaned 12_29_22_2.csv", row.names = F)
