...
##read in data
master = read.csv("0-Data/Merged_USM_Batch7.csv", stringsAsFactors = F)
...
dat$affordance_response[dat$affordance_response == "this doesnt have a use"] = NA
dat$affordance_response[dat$affordance_response == "is this a trick question?"] = NA
dat$affordance_response[dat$affordance_response == "snot"] = NA
...
no_stop$word[no_stop$word == "snot"] = NA

no_stop = na.omit(no_stop)
...
##Write to .csv
#write.csv(combined, file = "USM Cleaned batch 7.csv", row.names = F)
